...
将 .dsh 目录复制到 $DSH_HOME/.agent-presets/hermes-dsh-starter-kit/：

    # Windows PowerShell 7
    $dst = Join-Path $env:DSH_HOME ".agent-presets\hermes-dsh-starter-kit"
    Copy-Item -Recurse -Force ".\dsh" $dst

重启 DeepSeek Harness 后，在会话预设列表选择 Hermes DSH 快速上手。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 $DSH_HOME/skills/dsh-starter-kit/：

    $dst = Join-Path $env:DSH_HOME "skills\dsh-starter-kit"
    New-Item -ItemType Directory -Force -Path $dst | Out-Null
    Copy-Item ".\skills\dsh-starter-kit\SKILL.md" $dst -Force
